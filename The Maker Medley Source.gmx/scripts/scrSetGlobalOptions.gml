...
global.debugNoDeath = global.debugMode;      // enables god mode (toggle with Home key)
...
global.screenshotMode = false;   // used to get level/attack previews

global.roomCaptionDef = "The Maker Medley";  //sets default window caption (only works with the Professional version of Studio)
global.roomCaptionLast = global.roomCaptionDef;
window_set_caption(global.roomCaptionDef);

global.startRoom = rStage01Abstr4ktNew;    //stores which room we begin with
global.endRoom = rTMM_ClearLobby;    //stores which room is the last one
...
global.pauseDelayLength = 0; //40;   //sets the delay in frames in which the player can pause/unpause the game (can be set to 0 to disable pause delay)
